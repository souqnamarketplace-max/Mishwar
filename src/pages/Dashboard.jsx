...
function Overview() {
  ...
  const chartData = trips.slice(0, 7).map((trip, i) => ({
    name: new Date(trip.created_date).toLocaleDateString("ar"),
  ...
  const recentTrips = trips.slice(0, 4).map((trip) => ({
    ...
    date: new Date(trip.created_date).toLocaleDateString("ar"),
    time: trip.time,
    status: trip.status,
  }));
  return (
             ...
             <div key={n.id} className="flex items-start gap-3 p-3 rounded-lg hover:bg-muted/30 transition-colors">
               ...
               <div>
                 ...
                 <p className="text-[10px] text-muted-foreground mt-0.5 flex items-center gap-1">
                   <Clock className="w-3 h-3" />{new Date(n.created_date).toLocaleDateString("ar")}
                 </p>
               </div>
             </div>
           ))}
