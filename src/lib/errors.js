...
const KNOWN_PATTERNS = [
  ...
  // OAuth provider errors (Apple, Google) — these come from Supabase
  // as raw English strings that must never reach the user as-is.
  [/error updating user/i,
                                                           "تعذر تسجيل الدخول. حاول مجدداً أو استخدم البريد الإلكتروني"],
  [/invalid.*id.?token|id.?token.*invalid|id.?token.*expired/i,
                                                           "انتهت جلسة تسجيل الدخول — حاول مجدداً"],
  [/oauth.*error|error.*oauth|provider.*error|error.*provider/i,
                                                           "تعذر التواصل مع مزود الخدمة — حاول مجدداً"],
  [/popup.*closed|user.*closed.*popup|window.*closed/i,
                                                           "تم إغلاق نافذة تسجيل الدخول"],
  [/access_denied|access denied/i,
                                                           "تم رفض الإذن — تحقق من إعدادات حسابك"],
  [/identity.*already.*linked|already.*associated.*account/i,
                                                           "هذا الحساب مرتبط بطريقة تسجيل مختلفة — جرب بريدك الإلكتروني"],
  [/unverified.*email|email.*unverified/i,
                                                           "بريدك الإلكتروني غير مؤكد لدى Apple/Google"],
...
export function friendlyError(err, fallback = "حدث خطأ. حاول مجدداً") {
  ...
  // English message that didn't match a pattern — don't surface raw
  // English to Arabic users. Just return the Arabic fallback.
  // Previously we appended the English message for debugging, but that
  // caused strings like "تعذر تسجيل الدخول: Error updating user" to
  // reach users. All known Supabase/PostgREST errors now have Arabic
  // patterns above — anything falling through here is unexpected and
  // the generic fallback is better than raw English.
  return fallback;
}
